<x-app-layout title="{{$car->model->name}} {{$car->model->year}}">
    <main>
        <div class="container">
            <h1 class="car-details-page-title">{{$car->maker->name}} {{$car->model->name}} - {{$car->year}}</h1>
            <div class="car-details-region">{{$car->city->name ?? $car->municipality->name}} - {{$car->published_at}}</div>

            <div class="car-details-content">
                <div class="car-images-and-description">
                    <div class="car-images-carousel">
                        <div class="car-image-wrapper">
                            <img src="{{$car->primaryImage->image}}" alt="" class="car-active-image"
                                id="activeImage" />
                        </div>
                    ...
                    <div class="card car-detailed-description">
                        ...
                        <ul class="car-specifications">
                            <x-car-spec :value="$car->features->abs">
                                ABS
                            </x-car-spec>
                            <x-car-spec :value="$car->features->air_conditioning">
                                Air Conditioning
                            </x-car-spec>
                            <x-car-spec :value="$car->features->power_windows">
                                Power Windows
                            </x-car-spec>
                            <x-car-spec :value="$car->features->power_door_locks">
                                Power Door Locks
                            </x-car-spec>
                            <x-car-spec :value="$car->features->cruise_control">
                                Cruise Control
                            </x-car-spec>
                            <x-car-spec :value="$car->features->bluetooth_connectivity">
                                Bluetooth Connectivity
                            </x-car-spec>
                            <x-car-spec :value="$car->features->remote_start">
                                Remote Start
                            </x-car-spec>
                            <x-car-spec :value="$car->features->gps_navigation">
                                GPS Navigation
                            </x-car-spec>
                            <x-car-spec :value="$car->features->heated_seats">
                                Heated Seats
                            </x-car-spec>
                            <x-car-spec :value="$car->features->climate_control">
                                Climate Control
                            </x-car-spec>
                            <x-car-spec :value="$car->features->rear_parking_sensors">
                                Rear Parking Sensors
                            </x-car-spec>
                            <x-car-spec :value="$car->features->leather_seats">
                                Leather Seats
                            </x-car-spec>
                        </ul>
                    </div>
